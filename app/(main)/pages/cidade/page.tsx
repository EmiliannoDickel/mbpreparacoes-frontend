    ...
    const Cidade = () => {
        const ObjetoNovo: Projeto.Cidade = {  
            id: null,
            nome: '',
            estado: ''
        };

    const [objetos, setObjetos] = useState<Projeto.Cidade[]>([]);
    ...
    const [objeto, setObjeto] = useState<Projeto.Cidade>(ObjetoNovo); 
    const [selectedObjetos, setSelectedObjetos] = useState<Projeto.Cidade[]>([]);
    ...
    useEffect(() => {
       ...
       useEffect(() => {
                objetoService.buscarTodas()
                    ...
                    .catch((error) => {
                        ...
                        toast.current?.show({
                            severity: 'error',
                            summary: 'Erro',
                            detail: 'Não foi possível carregar os estados',
                            life: 3000
                        });
                    });
            }, []);
        ...
        const editObjeto = (objeto: Projeto.Cidade) => { //editar
            setObjeto({ ...objeto });
            setObjetoDialog(true);
        };

        const confirmDeleteObjeto = (objeto: Projeto.Cidade) => { //confirmar remocao
            setObjeto(objeto);
        ...
        const estadoBodyTemplate = (rowData: Projeto) => {
            return (
                <>
            ...
            <div className="grid crud-demo">
                <div className="col-12">
                    ...
                    >
                        ...
                        <Dialog visible={objetoDialog} style={{ width: '450px' }} header="Detalhes Da Cidade" modal className="p-fluid" footer={objetoDialogFooter} onHide={hideDialog}>
                            <div className="field">
                            ...
                            <div className="field">
                                ...
                                <Dropdown value={objeto.estado} onChange={(e) => onInputChange(e, 'estado')} options={estados} filter optionLabel="nome" 
    placeholder="Selecione um Estado" className="w-full md:w-14rem" />
                                {submitted && !objeto.estado && <small className="p-invalid">Estado é necessário.</small>}
                            </div>
                        ...
                        <Dialog visible={deleteObjetoDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deleteObjetoDialogFooter} onHide={hideDeleteObjetoDialog}>
                            <div className="flex align-items-center justify-content-center">
                                ...
                                {objeto && (
                                    <span>
                                    Você Realmente deseja excluir a cidade? <b>{objeto.nome}</b>?
                                    </span>
                                )}
                            </div>
                        </Dialog>
                        ...
                        <Dialog visible={deleteObjetosDialog} style={{ width: '450px' }} header="Confirmar" modal footer={deleteObjetosDialogFooter} onHide={hideDeleteObjetosDialog}>
                            <div className="flex align-items-center justify-content-center">
                                <i className="pi pi-exclamation-triangle mr-3" style={{ fontSize: '2rem' }} />
                                {objeto && <span>Você Realmente deseja excluir as cidades selecionados??</span>}
                            </div>
                        </Dialog>
                    </div>
                </div>
